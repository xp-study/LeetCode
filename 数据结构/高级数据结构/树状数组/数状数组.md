...
![在这里插入图片描述](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/1.jpg)
...
![在这里插入图片描述](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/2.jpg)

- 树状数组是多叉树

红色部分表示预处理数组，蓝色部分是原始输入数组，箭头表示当前值是哪些结点的值的和。

![在这里插入图片描述](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/3.jpg)

## 「树状数组」如何组织原始输入数据的结构

注意：和「堆」一样，树状数组的 0 号下标不放置元素，从 1 号下标开始使用。从上图可以观察到，与数组 C 的某个结点有关的数组 A 的某些结点，它们的下标之间有如下关系。

![image-20210626115015799](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/4.jpg)

这件事情是由下标数值的二进制决定的，把下标写成二进制的形式，最低位的 1 以及后面的 0 表示了预处理数组 C 管理了多少输入数组 A 的元素。我们看一下下面的图：

![在这里插入图片描述](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/5.jpg)

例如：6 的二进制表示为 01100110，这里只保留最低 4 位。将 6 进行二级制分解得到：

![image-20210626115124163](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/6.jpg)

最后的这部分![image-20210626115234194](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/7.jpg)决定了 C[6] 管理了多少个输入数组 A 的数据，这里是 2 个，即从下标 6 开始（包括 6）向前数 2 个数，因此 C[6] = A[5] +A[6]，其它同理。
...
我们使用记号 `preSum[7]` 表示查询 `A[1] + A[2] + ... + A[7]`。依然是考虑 7 的二进制![image-20210626115531553](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/8.jpg)分解：

![image-20210626115603454](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/9.jpg)

这三部分可以看成![image-20210626115650765](http://gitlab.wsh-study.com/xp-study/LeeteCode/blob/master/数据结构/高级数据结构/树状数组/images/数状数组/10.jpg)  这 3 部分之和，分别表示 4 个元素 + 2 个元素 + 1 个元素，正好是 lowbit 值一直减，减到 0 为止，每减去一个 lowbit 值，消去一个 1。
