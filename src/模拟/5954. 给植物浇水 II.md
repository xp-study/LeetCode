...
class Solution {
    public int minimumRefill(int[] plants, int capacityA, int capacityB) {
        ...
        int remainedA = capacityA;
        int remainedB = capacityB;
        while (i < j) {
            if (remainedA < plants[i]) {
                remainedA = capacityA;
                ans++;
            }
            remainedA -= plants[i];

            if (remainedB < plants[j]) {
                remainedB = capacityB;
                ans++;
            }
            remainedB -= plants[j];
        ...
        if (remainedA >= remainedB && remainedA < plants[i]) {
            ans++;
        } else if (remainedA < remainedB && remainedB < plants[i]) {
            ans++;
        }
